 ...
 * @param {Object} props.itemData - Details of a single inventory item used to populate the label and barcode
...
const RollBarcodeCard = ({ itemData }) => {
  ...
  useEffect(() => {
    ...
    const barcodeValue = data?.rollNo ? String(data.rollNo) : '';
    if (barcodeValue && el) {
      el.innerHTML = "";
      try {
  ...
  const barcodeText = data.rollNo ? String(data.rollNo) : '';
  const chars = barcodeText.split('');

  return (
    <div className="flex flex-col items-center w-full px-2 overflow-hidden">
      <div className="w-full flex flex-col items-center overflow-x-auto py-4 gap-4">
        <div className="roll-barcode-card-item w-[4in] h-[2in] p-2 border-2 border-black bg-white text-[7.5pt] font-sans font-bold flex flex-col justify-between shrink-0 select-none">
