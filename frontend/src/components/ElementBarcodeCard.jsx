 ...
 * @description React component for rendering an inventory item label card with a generated barcode and PDF download functionality.
 */
 ...
 * @param {Object} props.itemData - Details of the inventory item used to populate the card and barcode
...
const BarcodeCard = ({ itemData }) => {
  ...
  // Generate Barcode using itemData.element instead of rollNo
  useEffect(() => {
    if (itemData?.element && barcodeRef.current) {
      barcodeRef.current.innerHTML = "";
      try {
        JsBarcode(barcodeRef.current, itemData.element, {
          format: "CODE128", 
          width: 1.5, 
          height: 30, 
          displayValue: false, 
  ...
  const generatePDF = useCallback(async () => {
    if (!cardRef.current || !itemData?.element) return;
    ...
    try {
      ...
      pdf.save(`Label_Element_${itemData.element}.pdf`);
    } catch (err) {
      ...
      <div className="w-full flex justify-center overflow-x-auto py-4">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 mt-1 flex-grow">
            ...
            <div className="col-span-2 font-bold"><strong>Roll No:</strong> {itemData.rollNo || 'N/A'}</div>
          </div>
